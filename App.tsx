...
const App: React.FC = () => {
        ...
        const nextTasks = prevTasks.map(task => {
          if (task.status === TaskStatus.RUNNING) {
            ...
            if (newElapsed >= task.targetDuration) {
              return { ...task, elapsedSeconds: task.targetDuration, status: TaskStatus.COMPLETED };
            }
            return {
              ...task,
  ...
  const toggleTimer = (id: string) => {
    setTasks(prev => prev.map(t => {
      if (t.id === id) {
        const newStatus = t.status === TaskStatus.RUNNING ? TaskStatus.PAUSED : TaskStatus.RUNNING;
        return { ...t, status: newStatus };
      }
      return t.status === TaskStatus.RUNNING ? { ...t, status: TaskStatus.PAUSED } : t;
    }));
  };
